...
// Verse text is immutable Bible content — safe to cache aggressively at the
// CDN/browser. SWR window covers occasional schema bumps without forcing a
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { reference: formattedRef, text, translation: 'BSB' },
      { headers: CACHE_HEADERS }
    );
  } catch (err) {
    logger.error({ route: '/api/verses', err }, 'Verse lookup failed');
