...
function buildInMemoryDb(): Database.Database {
  ...
  `).run()
  db.prepare("INSERT INTO users VALUES ('u1')").run()
  return db
}
...
describe('patchUserFlags', () => {
  ...
  it('upserts has_seen_bench_intro', () => {
    patchUserFlags('u1', { has_seen_bench_intro: 1 })
    expect(getUserFlags('u1').has_seen_bench_intro).toBe(1)
  })

  it('upserts has_drawn_first_connection', () => {
    patchUserFlags('u1', { has_drawn_first_connection: 1 })
    expect(getUserFlags('u1').has_drawn_first_connection).toBe(1)
  })
