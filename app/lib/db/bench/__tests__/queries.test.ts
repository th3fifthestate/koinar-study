...
describe('getBenchBoard', () => {
  it('returns null when board not found', () => {
    ...
    expect(getBenchBoard('board-1', 'user-1')).toBeNull()
  })

  it('returns board when user owns it', () => {
    const board = { id: 'board-1', user_id: 'user-1' }
    mockGet.mockReturnValue(board)
    expect(getBenchBoard('board-1', 'user-1')).toEqual(board)
  })
})

describe('createBenchBoard', () => {
  it('runs INSERT and returns new board via get', () => {
    const board = { id: 'b1', user_id: 'u1', title: 'My Board' }
    mockGet.mockReturnValue(board)
    const result = createBenchBoard('u1', 'My Board')
...
describe('updateBenchBoard', () => {
  it('builds SET clause from patch keys only', () => {
    ...
    updateBenchBoard('b1', 'u1', { title: 'New Title' })
    const sql = mockPrepare.mock.calls.find(
      (c) => typeof c[0] === 'string' && (c[0] as string).includes('UPDATE bench_boards')
    )?.[0] as string
...
describe('deleteBenchBoard', () => {
  it('deletes by id and user_id', () => {
    deleteBenchBoard('b1', 'u1')
    expect(mockRun).toHaveBeenCalledWith('b1', 'u1')
  })
})
...
describe('updateBenchClipping', () => {
  it('builds SET clause from patch keys', () => {
    ...
    const sql = mockPrepare.mock.calls.find(
      (c) => typeof c[0] === 'string' && (c[0] as string).includes('UPDATE bench_clippings')
    )?.[0] as string
...
describe('getRecentVerseSeeds', () => {
  it('queries bench_clippings for verse type ordered by created_at desc', () => {
    mockAll.mockReturnValue([])
    getRecentVerseSeeds('user-1', 10)
    const sql = mockPrepare.mock.calls.at(-1)?.[0] as string
    expect(sql).toContain("clipping_type = 'verse'")
    expect(sql).toContain('LIMIT ?')
    expect(mockAll).toHaveBeenCalledWith('user-1', 10)
  })
})
