...
export const queryVerse = tool({
  ...
  execute: async ({ book, chapter, startVerse, endVerse }) => {
    try {
      ...
      console.error("[queryVerse] DB error:", err);
      return { error: "Database error retrieving verse." };
    }
  },
});
...
export const searchKeyword = tool({
  ...
  execute: async ({ query, limit }) => {
    ...
    try {
      ...
      verses =
        ftsResult.results.length > 0
          ? ftsResult.results
          : searchVerses(query, limit);
    } catch {
      // FTS DB unavailable — fall back to LIKE search
      try {
        verses = searchVerses(query, limit);
      } catch (err) {
        console.error("[searchKeyword] DB error:", err);
...
export const lookupStrongsNumber = tool({
  ...
  execute: async ({ strongsNumber }) => {
    try {
      ...
      console.error("[lookupStrongsNumber] DB error:", err);
      return { error: "Database error retrieving Strong's entry." };
    }
  },
});
...
export const originalLanguage = tool({
  ...
  execute: async ({ book, chapter, verse }) => {
    try {
      ...
      console.error("[originalLanguage] DB error:", err);
      return { error: "Database error retrieving original language data." };
    }
  },
});
...
export const lookupCrossReferences = tool({
  ...
  execute: async ({ book, chapter, verse, limit }) => {
    try {
      ...
      console.error("[lookupCrossReferences] DB error:", err);
      return { error: "Database error retrieving cross-references." };
    }
  },
});
